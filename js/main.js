...
const exportDocxBtnClick = () => {
    ...
    createGraphPng().then(
        img => {
            ...
            for (let i = 0; i != keys.length; i++) {
                ...
                if (prevNodeHelpIndex != nodeHelpIndex) {
                    if (prevNodeHelpIndex != -1) {
                        divToExport.append(msWordPageBreak + '<div class=\'node-help-head\'>' + nodeHelpLabels.join(' | ') + '</div>');
                    ...
                    nodeHelpLabels = [];
                    nodeHelp = converter.makeHtml(htmlHelpContent[nodeHelpIndex]);
            ...
            let blob = htmlDocx.asBlob(toExport, { size: 'A4' });
